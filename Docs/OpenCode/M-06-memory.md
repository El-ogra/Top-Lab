...
|---|-------------|--------|-----------------|
| 1 | Domain: `CultureResult.Update`, `MedicalConditionCategory.Pregnancy` + seed, `PregnancySignal` helper contract (+ first-shipper contingencies) + tests | [ ] Not started | VG-01 |
| 2 | Application read surface: culture entry grid (display-filtered) + culture report DTO | [ ] Not started | VG-02 |
| 3 | Application write surface: save culture result + sensitivities (replace-list), verify, unverify, print + authorization tests | [ ] Not started | VG-03 |
...
- [ ] **Stage 1 — Pre-Execution Verification:** Build passes `zero errors + zero warnings` and all tests pass. Evidence: run `dotnet build TopLab.sln` + `dotnet test TopLab.sln`.
- [ ] **Stage 2 — Deep Understanding:** Requirements, inputs, outputs, edge cases documented. Notes: plan §5.5; migration-scope gate: model vs. snapshot — expected zero drift apart from the «حمل» catalog seed row if the seed mechanism produced one in Slice 1 (then exactly that one flagged migration); any other drift → stop + addendum; ADR-0037 records: settled attached-only entry model (S-11 + FR-M06-002 + Data Model §6.4 + FR-M15-002 + ADR-0031), display-filter-at-entry-only, facts-never-hidden, replace-list pin; owner-settled pregnancy storage: `MedicalConditionCategory.Pregnancy = 2` + seeded «حمل» `MedicalConditionType` + structural category read (and the Data Model §4.2-vs-§13 conceptual/physical discrepancy resolved by this decision — recorded for the data-model owner); `MarkEntered` entered-invariant mechanism (owner-settled); first-shipper outcome; tracking-sheet M06 row verified at line 75.
- [ ] **Stage 3 — File Analysis:** Every file this slice touches listed and inspected. Files: `F5ConfigurationTests.cs`, `ApplicationDbContextModelSnapshot.cs` (relevant sections), `CultureResultConfiguration.cs`, `CultureAntibioticResultConfiguration.cs` (FK matrix), `AntibioticConfiguration.cs`, M15's `CultureAntibioticAttachment` (composite PK), InMemory harness, `Top_Lab_ADR.md`, `Top_Lab_Master_Tracking_Sheet.md`, `Top_Lab_Handoff_Template.md` / `Handoff_M12.md` (template).
- [ ] **Stage 4 — Planning:** Step-by-step execution plan written. Plan: run migration-scope gate first (model vs. snapshot; expect zero drift apart from flagged seed migration if any) → extend `F5ConfigurationTests` for `CultureResult`/`CultureAntibioticResult` FK matrix + seeded «حمل» row → create `CultureResultPersistenceTests` (InMemory upsert + replace + cascade + Restrict) → ADR-0037 → tracking flip → handoff.
- [ ] **Stage 5 — Execution:** Slice implemented per plan.
- [ ] **Stage 6 — Post-Execution Verification:** Build + tests pass again `zero errors + zero warnings`.
- [ ] **Stage 7 — Validation Gate:** VG-04 passed. Evidence: Release build 0/0, full suite green, drift limited to the flagged seed migration (if any) or zero-drift proven, coverage floors met or waivers documented, zero Presentation content (grep gate).
- [ ] **Stage 8 — Documentation Update:** Every checkbox in this slice marked [x] where applicable.
- [ ] **Stage 9 — Memory Status Update:** "Current Status" section updated; module close-out recorded.
- [ ] **Stage 10 — Git Commit (authorized local):** `[M-06] Slice 4/4: Infrastructure proof + close-out — loop-engineering` + `Stages 1-10 verified. Gate VG-04 passed.` — on `main`, never push.
