...
| M05 | Specialized Profile Result Reports | 6 | 🟩 | Done | Local coding agent (Top-Lab) | M04, M12 | 2026-09-09 | 2026-09-09 | Delivered: profile catalog (Analyte owns the only live range; `ProfileAnalyte` composition; `Profile` 1:1 with a `ResultKind.SpecializedProfile` test) + `ProfileResultItem` + 1:1 frozen-range snapshot (only migration `AddAnalyteProfileDomain`) + `ProfileResultAmendment` immutable audit history; read surface (entry grid, frozen-range report/reprint reading snapshots only with `ReportName` display, amendments history under `PT_AUDIT_ACCESS`); write surface (draft async save with per-item band capture + single save, verify/unverify guarded, print with reused M-04 `BalanceProbe` per-user block + absolute bypass, atomic post-print `AmendProfileResult` updating the active row and appending the audit row in one `SaveChangesAsync`); central pricing (`ManualSelectionCharge`/`ProfileSelectionCharge` — no feature-local formula); feature-local `DomainFailureTranslator`; VG proof (EF config FK matrix incl. Restrict on item→Analyte, InMemory relational rollback proof via failing `ISaveChangesInterceptor`, reprint-after-live-range-change across contexts, real-context central-charge round-trip); zero drift but `AddAnalyteProfileDomain` applied; no `PermissionConfiguration`, no DI, no Presentation; full suite green; Release build 0/0; ADR-0036. |
| M06 | Culture & Sensitivity Result Entry | 6 | 🟩 | Done | Local coding agent (Top-Lab) | M04, M15 | 2026-09-11 | 2026-09-11 | Delivered: backend for culture & sensitivity result entry — attached-only save with replace-list sensitivities, verify/unverify lifecycle gates, balance-blocked print (S-11 + FR-M06-002 + ADR-0031); owner-settled pregnancy storage (`MedicalConditionCategory.Pregnancy = 2` + seeded «حمل» row); F5 FK-matrix + InMemory persistence proof; zero drift; zero Presentation; full suite green 1395; Release build 0/0; ADR-0037. |
| M08 | Patient Search, Lab ID & Visit History | 6 | 🟩 | Done | Local coding agent (Top-Lab) | M02, M04 | 2026-09-11 | 2026-09-11 | Delivered: Application-only read surface — 4 unauthorized queries (global multi-criteria search incl. any stored phone + exact-trimmed LabId/NationalId channels + name-assist literal, Lab-ID lookup, visit history with settled `!IsDeleted` LabId row-grouping + rollup, visit detail with flat-set phones/conditions + settled balance); feature-local `BalanceProbe` (domain-calculator delegation) + `VisitRollup`; settings echo with asymmetric missing-settings policy; authorization-shape test pins no `IAuthorizedRequest` on the 4 read queries; settings-dependency round-trip (no write surface — documented); zero drift (no migration); zero Presentation; full suite green 1431; Release build 0/0; ADR-0038. |
| M11 | Work Sheets | 6 | 🟩 | Done | Local coding agent (Top-Lab) | M02, M12 | 2026-09-11 | 2026-09-11 | Delivered: period/range-based work sheet backend (FR-M11-001/002/003 + S-33 + FR-OUT-07) + in-module test-count classification (FR-M11-004); 4 queries gated `PRINT_WORKSHEET` (by WorkGroupLog, by TestGroup, Summary, TestCountByPeriod) with owner-settled `LabId`+`PatientTestId` line identifier pair, period defaulting (both null → today UTC, `From > To` → Validation, inclusive bounds), outside-lab excluded from bench sheets but included in FR-M11-004 count; DTO-as-worksheet (no renderer — `IBarcodeService`/`IReportPrintingService` unimplemented per Reporting §13); 3 system settings echoed; Infrastructure proof (F5 `WorkGroupLog`/`WorkGroupLogItem` mapping + InMemory persistence tests for by-log filtering + classification outside-lab inclusion); zero drift; zero migration; zero Presentation; full suite green 1462 (360 + 975/976 waived flake + 126); Release build 0/0; ADR-0039. Commits `12dbef0` (S1) + S2 close-out commit. |
| M07 | Combined, Blank & History Reports | 7 | 🟩 | Done | Local coding agent (Top-Lab) | M04, M05, M06, M22 | 2026-09-15 | 2026-09-15 | Delivered: four report types (combined/blank/patient-history/separate+multi-patient history) across Domain+Application+Infrastructure; PDF-first `IReportPrintingService` implementation (OD-07-A) with settings-at-print-time + temp-file dispatch; ephemeral `CombinedReportSelection` (OD-07-B) + `PatientHistoryResolver` (OD-07-C ByLabCode/exact-normalized-name); `PRINT_RESULTS`-gated print commands with BR-07 balance gate on combined+history only (OD-07-E, blank exempt); `MarkPrinted` audit per printed line; history insertion (auto/manual) as DTO copies, no stored-row mutation; zero drift (no migration); zero Presentation; full suite green 1581; Release build 0/0; ADR-0040. Commits `9e4c84e` (S1) + `d6623e1` (S2) + `664ed34` (S3) + `a269e99` (S4) + S5 close-out. |
| M09 | Result Delivery & Settlement at Handover | 7 | 🟩 | Done | Local coding agent (Top-Lab) | M04, M03, M17 | 2026-09-15 | 2026-09-15 | Delivered: delivery-handover aggregation surface — period-filtered undelivered list (per-visit status via `PatientStatusCalculator` verbatim) + per-patient delivery grid with frozen `PriceAtOrderTime` + financial position delegated to `PatientBillingReader` with sign-derived remaining-to-lab/to-patient + composite `DeliverWithSettlement` (printed-only delivery with per-line audit, partial `Payment` default or delegated "خلاص" full settlement, single save, NO balance gate — BR-08 pinned); all 4 use cases gated `DELIVER_RESULTS`; zero Domain changes; zero drift (no migration); zero Presentation; full suite green 1632; Release build 0/0; ADR-0041. Commits `be21e9e` (S1) + `025b2d0` (S2) + S3 close-out. |
| M16 | Sent-Out Samples | 7 | 🟩 | Done | Local coding agent (Top-Lab) | M02, M12, M14, M03 | 2026-09-15 | 2026-09-15 | Delivered: Domain guards on `SentOutSample.Create` (cost/patient ≥ 0) + `SentOutSamplePayment` (amount > 0) + pure static `SentOutAccountCalculator` (single-sourced §8.3 formula); write surface (3 `CASH_DISBURSE_DEPOSIT`-gated commands: `SendSampleOut` with eligibility/type/duplication guards + default-price capture with override, `RecordSentOutPayment` capped at remaining, `SettleSentOutInFull` exact-remaining «خلاص») + read surface (2 open queries: period `GetSentOutSamples` with optional entity filter + per-lab `GetSentOutLabAccount` with dispatch count and calculator totals); feature-local access policy + `DomainFailureTranslator`; per-sample lab binding (OD-16-A, PRD contradiction recorded in ADR-0042), no cancel/re-send v1 (OD-16-B); `FakeApplicationDbContext.SentOutSamplePayments` extension (proven gap); InMemory persistence proof (store/retrieve + Cascade + Restrict pins + audit gate); 5 validator-registration cases; zero drift (no migration); zero Presentation; full suite green 1696; Release build 0/0; ADR-0042. Commits `3b499c5` (S1) + `f6db46d` (S2) + `9d4c88a` (S3) + S4 close-out. |
| M10 | Case Tracking, Audit & Traceability (P/T) | 8 | 🟩 | Done | Local coding agent (Top-Lab) | M02, M03, M04, M09, M17 | 2026-09-15 | 2026-09-15 | Delivered: Application-only restricted read surface in `AuditAndTraceability` feature folder (no Domain/Infrastructure changes, no migration); 2 `IAuthorizedRequest` queries gated `PT_AUDIT_ACCESS` id=13 via feature-local `AuditAccessPolicy` (authorized-reads exception per FR-M17-008, absolute bypass via pipeline) — `GetPatientAuditQuery` P view (registering user, `ModificationCount`, last modifier, distinct time-ordered payment receivers incl. voided rows, soft-deleted patients included, unknown → `المريض غير موجود.`) + `GetPatientTestAuditQuery` T view (entered/reviewed/printed+`PrintCount`/delivered with users + UTC times, partial lifecycles → nulls, unknown → `التحليل غير موجود`); dictionary name resolution with raw-id fallback; 3 validators auto-registered (2 query validators + registration theory); zero drift (no migration, snapshot untouched); zero Presentation; full suite green 1715; Release build 0/0; ADR-0043. Commits `77c7d5e` (S1) + `acb99d4` (S2) + S3 close-out. |
| M18 | Attendance & Time Tracking | 8 | 🟩 | Done | Local coding agent (Top-Lab) | M17 | 2026-09-15 | 2026-09-15 | Delivered: Domain guards on `AttendanceRecord` (break sequencing + double check-out, English `InvalidOperationException`) + pure static `AttendanceCalculator` (lateness/overtime vs configured hours via local wall-clock, null when unconfigured; worked minutes exclude break span); write surface (4 ungated session-bound commands: `CheckIn` with single-open-record Conflict + computed lateness, `StartBreak`/`EndBreak` with open-record NotFound + guard translation, `CheckOut` with computed overtime; all unauthenticated → shared Forbidden); read surface (2 absolute-gated queries: period `GetAttendanceRecords` with optional user filter + dictionary names + half-open inclusive bounds + default-today, per-user `GetUserAttendanceSummary` with days/records/worked/lateness/overtime totals via calculator, unknown user → `المستخدم غير موجود.`); feature-local `DomainFailureTranslator`; no edit/delete path; `FakeApplicationDbContext` verify-only (no gap); InMemory persistence proof (store/retrieve full lifecycle + behavioral Cascade + model Cascade/index pins + not-auditable pin); 6 validator-registration cases; zero drift (no migration, snapshot untouched); zero Presentation; full suite green 1789; Release build 0/0; ADR-0044. Commits `3070c6e` (S1) + `908f05c` (S2) + `e686dfa` (S3) + S4 close-out. |
| M19 | Statistics | 8 | 🟩 | Done | Local coding agent (Top-Lab) | M02, M04, M14, M16, M17, M18 | 2026-09-15 | 2026-09-15 | Delivered: Application-only gated read surface in `Statistics` feature folder (zero Domain, zero Persistence, no migration); 4 `IAuthorizedRequest` queries gated `STATISTICS` id=12 via feature-local `StatisticsAccessPolicy` — `GetPatientCountStatistics` (sex/referral/account-type classifications, optional monthly + month×sex cross-buckets, soft-deleted excluded, null-referral «بدون جهة إحالة»), `GetTestCountStatistics` (per-test + per-group counts on `PatientTest.CreatedAtUtc`, optional group filter, deleted-patient exclusion, unknown group NotFound), `GetSentOutStatistics` (per-lab counts + totals via `SentOutAccountCalculator` only, optional lab filter), `GetUserProductivityStatistics` (per-timestamp attribution counts, zero-activity omitted, raw-id fallback); uniform half-open UTC period + default-today + inverted-period frozen message; 4 validator-registration cases; zero drift (no migration, snapshot untouched); zero Presentation; full suite green 1839; Release build 0/0; ADR-0045. Commits `c68e25c` (S1) + `cf8566e` (S2) + `9defb1b` (S3) + S4 close-out. |
| M20 | Inventory & Lab Accounting | 9 | 🟩 | Done | Local coding agent (Top-Lab) | M02, M03, M04, M09, M16, M14, M17, M22 | 2026-09-15 | 2026-09-15 | Delivered: `CashMovement.Create` guards (amount > 0, notes ≤ 500, occurredAtUtc set — behavior-only, zero schema change); `CASH_DISBURSE_DEPOSIT` id=11-gated Accounts surface in `Features/InventoryAndAccounting` — `GetCashDrawerInventory` (ten FR-M20-002 figures via `PatientAccountCalculator`/`SentOutAccountCalculator`, SafeCash/NetProfit per SD-20-10, commissions per SD-20-8), `GetElementInventory` (user/referral/doctor/account-type/sent-out × four report types, NotFound frozen messages), `GetPatientSamplesDetail` (FR-M20-005 drill-down), `RecordCashDeposit`/`RecordCashDisbursement` (handler-stamped performer/timestamp, entity NotFound), `ListCashMovements` (dictionary names + raw-id fallback), `GetCompanyDelegateAccounts` (per-entity cash + sent-out settlement); soft-deleted excluded; uniform half-open UTC periods; 7 validator-registration cases; zero drift (no migration, snapshot untouched); zero Presentation; full suite green; Release build 0/0; ADR-0046. Commits `7072143` (S1) + `69b7712` (S2) + `764efb0` (S3) + S4 close-out. |
| M23 | Utilities (Tools) | 10 | 🟩 | Done | Local coding agent (Top-Lab) | M01 | 2026-09-15 | 2026-09-15 | Delivered: pure Domain computation services (`MeasurementUnitConverter` explicit pair table, `ArithmeticCalculator` recursive-descent parser, `StopwatchCalculator.Elapsed`); ungated Application queries (`ConvertMeasurementUnit`, `EvaluateCalculation`, `ComputeStopwatchElapsed` via `IDateTimeProvider`, `GetTestLibrary` catalog with dictionary group names + unknown-group NotFound); workstation-local JSON lists (`IPurchasesListStore`/`IPhoneBookStore` + `JsonPurchasesListStore`/`JsonPhoneBookStore` under `%ProgramData%\TopLab`, two DI registrations); purchases add/remove/toggle + phone book add/remove + list queries; Image Library and Shortcut Library backends excluded (SD-23-6/7); 11 validator-registration cases; zero EF migration (drift clean every slice); zero Presentation; full suite green; Release build 0/0; ADR-0047. Commits `1fd8263` (S1) + `2ac7ea9` (S2) + `07a22b0` (S3) + S4 close-out. |
...
| Wave 6 — Result Specializations & Search | M05, M06, M08, M11 | ⬜ Not Started — M05 🟩 Done, M06 🟩 Done, M08 🟩 Done, M11 pending |
| Wave 7 — Report Production & Handover | M07, M09, M16 | ⬜ Not Started |
| Wave 8 — Audit, Attendance & Statistics | M10, M18, M19 | 🟩 All Done |
| Wave 9 — Financial Consolidation | M20 | 🟩 All Done |
| Wave 10 — Utilities | M23 | 🟩 All Done |
...
- Wave: 6
- Owner: Local coding agent (Top-Lab)
- Dependencies satisfied? Requires M02, M04 — satisfied.
- Implementation status: 🟩
- Current phase: Done
- Started: 2026-09-11
- Completed: 2026-09-11
- Notes: Application-only read surface (`PatientSearch` feature folder, no Domain/Infrastructure changes, no migration, no DI change): `SearchPatientsGlobalQuery` (multi-criteria: exact-trimmed LabId/NationalId channels, any stored phone via `PatientPhoneNumber`, assist-gated name search, exempt-filled/paginated; each hit carries the settled `AggregateStatus` + `TestCount`), `GetPatientByLabIdQuery` (NotFound `لا يوجد مريض بهذا الكود.`), `GetVisitHistoryQuery` (LabId row-grouping with `!IsDeleted` sibling filter, rollup via `VisitRollup`, worked-example balance, settings echo with asymmetric missing-settings policy), `GetVisitDetailQuery` (flat-set phones/conditions + test lines + `BalanceProbe`); `PatientSearchAuthorizationTests` pins all four queries carry no `IAuthorizedRequest`; no write surface (settings-dependency round-trip test documents the read-only scope); Infrastructure proof (`PatientSearchPersistenceTests` InMemory 2-siblings + unrelated-excluded; zero model drift); full suite green 1431; Release build 0/0; ADR-0038. Documentation: `Docs/Handoff_M08.md`.
...
- Implementation status: 🟩
- Current phase: Done
- Notes: `SentOutSample`, `SentOutSamplePayment`; per-lab follow-up and settlement. Delivered 2026-09-15: Domain guards + `SentOutAccountCalculator`; 3 gated write commands + 2 open read queries; per-sample lab binding; no cancel/re-send v1; zero drift; ADR-0042; `Docs/Handoff_M16.md`.
...
- Implementation status: 🟩
- Current phase: Done
- Notes: restricted `P` and `T` inspection queries; access via authorization pipeline. Delivered 2026-09-15: `GetPatientAuditQuery` P view + `GetPatientTestAuditQuery` T view, both gated `PT_AUDIT_ACCESS` (authorized-reads exception); voided operations included; soft-deleted patients auditable; zero drift; ADR-0043; `Docs/Handoff_M10.md`.
...
| 2026-09-09 | M05 | Specialized Profile Result Reports implemented across S1–S4: profile catalog (Analyte-owned live range, `ProfileAnalyte` composition, `Profile` 1:1 `SpecializedProfile` test) + `ProfileResultItem` + 1:1 frozen-range snapshot + immutable `ProfileResultAmendment` history (only migration `AddAnalyteProfileDomain`); read surface (`GetProfileEntryGrid`, frozen-range `GetProfileReport` reading snapshots only with `ReportName` display, `GetProfileResultAmendments` under `PT_AUDIT_ACCESS`); write surface (`SaveProfileResults` draft-only with per-item band capture + single save, `VerifyProfileResults`/`UnverifyProfileResults`, `MarkProfilePrinted` reusing the M-04 `BalanceProbe` block + absolute bypass, atomic `AmendProfileResult` active-row + audit-row in one `SaveChangesAsync`); central pricing via `ManualSelectionCharge`/`ProfileSelectionCharge` (no feature-local formula) wired through `AddProfileToVisit`; feature-local `DomainFailureTranslator`; VG proof (EF config FK matrix incl. Restrict on item→Analyte, InMemory atomic-rollback proof via failing `ISaveChangesInterceptor`, reprint-after-live-range-change across contexts, real-context central-charge round-trip); zero model drift against the migration; no `PermissionConfiguration`, no DI, no Presentation; full suite green; Release build 0/0. ADR-0036. Slice commits `2283841`, `17ba790`, `5cf6946` + the Slice 4 close-out commit. | Local coding agent (Top-Lab) |
| 2026-09-11 | M06 | Culture & Sensitivity Result Entry implemented across S1–S4: domain `CultureResult.Update` (trim + whitespace-to-null) + `MedicalConditionCategory.Pregnancy = 2` + seeded «حمل» `MedicalConditionType` (`Category = Pregnancy`) + `PregnancySignal` (first-shipper contingency not invoked — `PatientTest.MarkEntered`/`Unreview` already present); read surface (2 queries — `GetCultureEntryGrid` display-filtered at entry with facts-never-hidden, `GetCultureReport` saved-results-only with echoed print-code setting); write surface (4 commands — `SaveCultureResults` attached-only + replace-list + empty-allowed + length caps, `VerifyCultureResult`/`UnverifyCultureResult` lifecycle-gated with printed-parent guard, `MarkCultureReportPrinted` with the M-04 `BalanceProbe` per-user block + absolute bypass) gated `EDIT_RESULTS`/`REVIEW_RESULTS`/`PRINT_RESULTS`; 3 validators auto-registered (no DI change); sole flagged migration `20260910213833_AddPregnancyMedicalConditionTypeSeed`; F5 FK-matrix tests + InMemory persistence proof (replace-list, cascade graph, Restrict-on-antibiotic-delete at model level); zero model drift; zero Presentation; full suite green 1395 (360 Domain + 913 Application + 122 Infrastructure); Release build 0/0; ADR-0037. Slice commits `b614b89`, `0663d88`, `8b57439` + the Slice 4 close-out commit. | Local coding agent (Top-Lab) |
| 2026-09-11 | M08 | Patient Search, Lab ID & Visit History implemented across S1–S3: Application-only read surface in `PatientSearch` feature folder (no Domain/Infrastructure changes, no migration, no DI change) — global multi-criteria search (exact-trimmed `LabId`/`NationalId` channels, any stored phone, assist-gated name search, per-hit settled `AggregateStatus` + `TestCount`), Lab-ID lookup (NotFound `لا يوجد مريض بهذا الكود.`), visit history with settled `!IsDeleted` LabId row-grouping + `VisitRollup` (worked-example balance Charged 170 / Paid 90 / Balance 80, settings echo with asymmetric missing-settings policy — search default-false, history Unexpected), visit detail with flat-set phones/conditions + `BalanceProbe`; 4 unauthorized queries + 3 validators auto-registered; authorization-shape test pins all four queries carry no `IAuthorizedRequest`; settings-dependency round-trip (no write surface — documented); Infrastructure proof (InMemory `PatientSearchPersistenceTests`: two registrations sharing a LabId + an unrelated registration excluded; existing `(LabId)` + M-02 `IsDeleted` index re-assertions in `F5ConfigurationTests`; zero model drift); zero Presentation; full suite green 1431 (360 + 948 + 123); Release build 0/0; ADR-0038. Slice commits `c3bef9d`, `d73b3bc` + the Slice 3 close-out commit. | Local coding agent (Top-Lab) |
| 2026-09-11 | M11 | Work Sheets implemented across S1–S2: period/range-based work sheet backend (FR-M11-001/002/003 + S-33 + FR-OUT-07) + in-module test-count classification (FR-M11-004); 4 queries gated `PRINT_WORKSHEET` (by WorkGroupLog, by TestGroup, Summary, TestCountByPeriod) with owner-settled `LabId`+`PatientTestId` line identifier pair, period defaulting (both null → today UTC, `From > To` → Validation, inclusive bounds), outside-lab excluded from bench sheets but included in FR-M11-004 count; DTO-as-worksheet (no renderer — `IBarcodeService`/`IReportPrintingService` unimplemented per Reporting §13); 3 system settings echoed; Infrastructure proof (F5 `WorkGroupLog`/`WorkGroupLogItem` mapping + InMemory persistence tests for by-log filtering + classification outside-lab inclusion); zero drift; zero migration; zero Presentation; full suite green 1462 (360 + 975/976 waived flake + 126); Release build 0/0; ADR-0039. Commits `12dbef0` (S1) + S2 close-out commit. | Local coding agent (Top-Lab) |
| 2026-09-15 | M07 | Combined, Blank & History Reports implemented across S1–S5: Domain rules (`CombinedReportSelection` ephemeral + `PatientHistoryResolver` ByLabCode/exact-normalized-name); Application surface (6 commands + 4 queries + DTOs + access policy + `DomainFailureTranslator` + `PatientHistoryReader` + `HistoryInsertion`); Infrastructure `ReportPrintingService` (PDF-first, settings-at-print-time, temp-file dispatch, `PrinterAssignment` routing); `PRINT_RESULTS`-gated print commands with BR-07 balance gate on combined+history only (blank exempt); `MarkPrinted` audit per printed line; history insertion (auto/manual) as DTO copies, no stored-row mutation; zero drift (no migration); zero Presentation; full suite green 1581; Release build 0/0; ADR-0040. Commits `9e4c84e` (S1) + `d6623e1` (S2) + `664ed34` (S3) + `a269e99` (S4) + S5 close-out. | Local coding agent (Top-Lab) |
| 2026-09-15 | M09 | Result Delivery & Settlement at Handover implemented across S1–S3: Application read surface (period-filtered undelivered list with per-visit status via `PatientStatusCalculator` verbatim + delivery grid with frozen `PriceAtOrderTime` + financial position delegated to `PatientBillingReader` with sign-derived remaining-to-lab/to-patient, all 3 gated `DELIVER_RESULTS`); composite `DeliverWithSettlement` (printed-only delivery with per-line audit + partial `Payment` default or delegated "خلاص" full settlement, single save, no balance gate — BR-08 pinned); Infrastructure proof (InMemory persistence tests for period filter + `IsDeleted` exclusion + frozen price + audit columns after save); zero Domain changes; zero drift (no migration); zero Presentation; full suite green 1632; Release build 0/0; ADR-0041. Commits `be21e9e` (S1) + `025b2d0` (S2) + S3 close-out. | Local coding agent (Top-Lab) |
| 2026-09-15 | M16 | Sent-Out Samples implemented across S1–S4: Domain guards (`SentOutSample` non-negative prices, `SentOutSamplePayment` positive amount) + `SentOutAccountCalculator` (single-sourced §8.3 formula); Application write surface (`SendSampleOut` with full eligibility/type/duplication guard matrix + default-price capture with override, `RecordSentOutPayment` «ترسل إلى» capped at remaining, `SettleSentOutInFull` «خلاص» exact remaining, all 3 gated `CASH_DISBURSE_DEPOSIT` id=11); read surface (period `GetSentOutSamples` with optional entity filter + resolved names + per-sample totals, per-lab `GetSentOutLabAccount` with dispatch count + calculator totals, both open); Infrastructure proof (InMemory store/retrieve + Cascade payment→sample + Restrict sample→entity pins + interceptor audit gate); 5 validator-registration cases; per-sample lab binding (OD-16-A, FR-M16-001 PRD contradiction recorded) + no cancel/re-send v1 (OD-16-B); zero drift (no migration); zero Presentation; full suite green 1696; Release build 0/0; ADR-0042. Commits `3b499c5` (S1) + `f6db46d` (S2) + `9d4c88a` (S3) + S4 close-out. | Local coding agent (Top-Lab) |
| 2026-09-15 | M10 | Case Tracking, Audit & Traceability (P/T) implemented across S1–S3: Application-only restricted read surface in `AuditAndTraceability` feature folder (DTOs + `AuditAccessPolicy.PtAuditAccess` const + 2 queries with handlers and validators, no Domain/Infrastructure changes, no migration); `GetPatientAuditQuery` P view (registering user, `ModificationCount`, last modifier, distinct time-ordered payment receivers incl. voided rows, soft-deleted patients included, unknown → `المريض غير موجود.`) + `GetPatientTestAuditQuery` T view (entered/reviewed/printed+`PrintCount`/delivered with users + UTC times, partial lifecycles → nulls, unknown → `التحليل غير موجود`); both `IAuthorizedRequest` gated `PT_AUDIT_ACCESS` id=13 (authorized-reads exception per FR-M17-008, absolute bypass via pipeline); dictionary name resolution with raw-id fallback; 2 validator-registration cases; zero drift (`has-pending-model-changes` = no changes, snapshot untouched); zero Presentation; full suite green 1715; Release build 0/0; ADR-0043. Commits `77c7d5e` (S1) + `acb99d4` (S2) + S3 close-out. | Local coding agent (Top-Lab) |
| 2026-09-15 | M18 | Attendance & Time Tracking implemented across S1–S4: Domain guards on `AttendanceRecord` (break sequencing + double check-out) + pure static `AttendanceCalculator` (lateness/overtime vs `User` configured hours via local wall-clock, null when unconfigured; worked minutes exclude break span; overnight-shift documented behavior); Application write surface (4 ungated session-bound commands `CheckIn`/`StartBreak`/`EndBreak`/`CheckOut` with single-open-record Conflict, open-record NotFound, computed lateness/overtime, guard-to-Arabic translation, unauthenticated Forbidden) + read surface (2 handler-level absolute-gated queries: period `GetAttendanceRecords` with optional user filter + dictionary names + half-open inclusive bounds + default-today, per-user `GetUserAttendanceSummary` with days/records/worked/lateness/overtime totals); feature-local `DomainFailureTranslator`; no edit/delete path; Infrastructure proof (InMemory full-lifecycle store/retrieve + behavioral Cascade + model Cascade/index pins + not-auditable pin); 6 validator-registration cases; zero drift (no migration, snapshot untouched); zero Presentation; full suite green 1789; Release build 0/0; ADR-0044. Commits `3070c6e` (S1) + `908f05c` (S2) + `e686dfa` (S3) + S4 close-out. | Local coding agent (Top-Lab) |
| 2026-09-15 | M19 | Statistics implemented across S1–S4: Application-only gated read surface in `Statistics` feature folder (zero Domain, zero Persistence, no migration); 4 `IAuthorizedRequest` queries gated `STATISTICS` id=12 via feature-local `StatisticsAccessPolicy` — `GetPatientCountStatistics` (sex/referral/account-type classifications, optional monthly + month×sex cross-buckets, soft-deleted excluded, null-referral «بدون جهة إحالة»), `GetTestCountStatistics` (per-test + per-group counts on `PatientTest.CreatedAtUtc`, optional group filter, deleted-patient exclusion, unknown group → `مجموعة التحاليل غير موجودة.`), `GetSentOutStatistics` (per-lab counts + totals via `SentOutAccountCalculator` only, optional lab filter, unknown lab → `الجهة الخارجية غير موجودة.`), `GetUserProductivityStatistics` (per-timestamp attribution counts, zero-activity omitted, raw-id fallback, no M-18 coupling); uniform half-open UTC period + default-today + inverted-period frozen message; 4 validator-registration cases; zero drift (`has-pending-model-changes` = no changes, snapshot untouched); zero Presentation; full suite green 1839; Release build 0/0; ADR-0045. Commits `c68e25c` (S1) + `cf8566e` (S2) + `9defb1b` (S3) + S4 close-out. | Local coding agent (Top-Lab) |
| 2026-09-15 | M20 | Inventory & Lab Accounting implemented across S1–S4: `CashMovement.Create` guards (amount > 0, notes ≤ 500, occurredAtUtc set — behavior-only, zero schema change); `CASH_DISBURSE_DEPOSIT` id=11-gated Accounts surface — `GetCashDrawerInventory` (ten FR-M20-002 figures via calculators, never restated), `GetElementInventory` (user/referral/doctor/account-type/sent-out × four report types), `GetPatientSamplesDetail` drill-down, `RecordCashDeposit`/`RecordCashDisbursement` (handler-stamped performer/timestamp), `ListCashMovements`, `GetCompanyDelegateAccounts` (per-entity cash + sent-out settlement); soft-deleted excluded; uniform half-open UTC periods; 7 validator-registration cases; zero drift (no migration, snapshot untouched); zero Presentation; full suite green; Release build 0/0; ADR-0046. Commits `7072143` (S1) + `69b7712` (S2) + `764efb0` (S3) + S4 close-out. | Local coding agent (Top-Lab) |
| 2026-09-15 | M23 | Utilities (Tools) implemented across S1–S4: pure Domain computation services (`MeasurementUnitConverter` explicit pair table, `ArithmeticCalculator` recursive-descent parser, `StopwatchCalculator.Elapsed`); ungated Application queries (`ConvertMeasurementUnit`, `EvaluateCalculation`, `ComputeStopwatchElapsed` via `IDateTimeProvider`, `GetTestLibrary` catalog); workstation-local JSON lists (`IPurchasesListStore`/`IPhoneBookStore` + Infrastructure JSON stores under `%ProgramData%\TopLab`, two DI registrations); purchases add/remove/toggle + phone book add/remove + list queries; Image Library and Shortcut Library backends excluded (SD-23-6/7); 11 validator-registration cases; zero EF migration (drift clean every slice); zero Presentation; full suite green; Release build 0/0; ADR-0047. Commits `1fd8263` (S1) + `2ac7ea9` (S2) + `07a22b0` (S3) + S4 close-out. | Local coding agent (Top-Lab) |
