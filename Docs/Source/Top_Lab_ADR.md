...
### ADR-0038 — M-08: Patient Search, Lab ID & Visit History — dedicated read overlap with M-02, settled LabId grouping, and the permission-light read surface

- **Status:** Accepted
- **Date:** 2026-09-11

**Context.** Module 8 delivers the front-desk patient search (including any stored telephone number), the Lab-ID lookup, and the visit history for one registration, on top of the M-02 patient model. Every table it reads (`Patient`, `PatientPhoneNumber`, `PatientMedicalCondition`, `MedicalConditionType`, `PatientTest`, `PaymentOperation`) was already configured and exercised by M-02, and its indexes already exist — the `(LabId)` index from the F5 baseline and the M-02-owned `IsDeleted` index. Four binding decisions shape the module: how its read surface relates to M-02's handlers, how a multi-visit history groups rows, whether the four read queries carry an authorization surface, and how the FR-M08-007 aggregate status is produced per hit and per visit.

**Decision.**

1. **Dedicated richer read surface — no M-02 handler reuse.** M-08 re-queries the same tables with its own projections (`PatientSearchHitDto`, `VisitSummaryDto`, `VisitDetailDto`, `VisitTestLineDto`) and does not import or dispatch any M-02 handler or DTO. The M-02 read surface stays the registration-screen contract; the M-08 surface is the search/lookup/history contract. The overlap is deliberate and documented in this ADR — the feature-local grouping, rollup, and balance-reading rules exist only here, so the two surfaces can evolve independently.
2. **Exact-trimmed `LabId`/`NationalId` channels.** Global search matches `LabId` and `NationalId` by exact equality on trimmed values (unlike M-02's `.Contains` behavior); phone match covers any stored `PatientPhoneNumber` row; name search follows the settled `EnablePatientNameSearchAssist` literal.
3. **LabId row-grouping with the `!IsDeleted` sibling filter (beyond M-02).** Visit history for a non-null-LabId patient loads every `Patient` row sharing the raw-equal LabId value and excludes soft-deleted (`IsDeleted`) rows — a hardening M-02's history query does not apply; a null-LabId patient's history is its single visit, and the history emits `LabId = string.Empty` for that case.
4. **Flat-set reads.** Phone numbers and medical-condition names are read from the flat sets (`Set<PatientPhoneNumber>`, `Set<PatientMedicalCondition>`), not navigation collections, with deterministic ordering (phones by `SortOrder`, condition names by `MedicalConditionTypeId`, visit test lines by `PatientTestId`).
5. **Asymmetric settings policy.** The history queries echo `HistorySortMode`/`HistoryAutoDisplayEnabled` from the singleton `ReportSettings` row and return `Error.Unexpected("سجل إعدادات التقرير مفقود.")` when it is absent (mirroring M-22's `GetReportSettingsQueryHandler`); the search handler treats a missing `SystemSettings` row as `EnablePatientNameSearchAssist = false` (graceful default). The asymmetry is deliberate: search degrades safely, while history is a settings-consuming surface.
6. **Settled status via the Domain calculator.** Aggregate status comes exclusively from `PatientStatusCalculator` (ADR-0015). `PatientStatusCalculator.cs` already exists on `main`, so the first-shipper contingency was not invoked — the module imports the existing seven-state calculator and never re-implements or caches it.
7. **Permission-light reads.** All four queries implement plain `IRequest` and carry **no** `IAuthorizedRequest`; the authorization-shape test pins the absence. No `PermissionConfiguration` change.
8. **Zero-migration outcome.** The consumed indexes already exist — the `(LabId)` index is re-asserted by the existing `F5ConfigurationTests.Patient_HasExpectedColumns` and the M-02-owned `IsDeleted` index by `F5ConfigurationTests.Patient_HasIndexOnIsDeleted` (the module only re-asserts what it consumes; no new index, no schema touch). `dotnet ef migrations has-pending-model-changes` returns "No changes have been made to the model since the last migration".
