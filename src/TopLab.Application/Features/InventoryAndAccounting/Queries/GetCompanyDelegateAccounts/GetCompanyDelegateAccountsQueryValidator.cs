using FluentValidation;

namespace TopLab.Application.Features.InventoryAndAccounting.Queries.GetCompanyDelegateAccounts;

public sealed class GetCompanyDelegateAccountsQueryValidator : AbstractValidator<GetCompanyDelegateAccountsQuery>
{
    public GetCompanyDelegateAccountsQueryValidator()
    {
        RuleFor(x => x)
            .Must(q => !q.From.HasValue || !q.To.HasValue || q.From.Value <= q.To.Value)
            .WithMessage("بداية الفترة يجب ألا تتجاوز نهايتها.");
    }
}
